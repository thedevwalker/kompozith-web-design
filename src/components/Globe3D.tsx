...
export default function Globe3D() {
  ...
  const { visiblePoints, hiddenPoints, locationsOnSphere } = useMemo(() => {
    ...
    globePoints.forEach((pt, index) => {
      ...
      if (z >= 0) {
        // Front side - slate dots
        visible.push({
          key: `pt-v-${index}`,
          x: sx,
          y: sy,
          r: dotRadius,
          color: pt.isLand ? "rgba(71, 85, 105, 0.35)" : "rgba(71, 85, 105, 0.08)",
          opacity: pt.isLand ? 0.8 : 0.4,
        });
      } else {
        // Back side - faint slate dots
        hidden.push({
          key: `pt-h-${index}`,
          x: sx,
          y: sy,
          r: dotRadius * 0.8,
          color: pt.isLand ? "rgba(71, 85, 105, 0.07)" : "rgba(71, 85, 105, 0.02)",
          opacity: 0.2,
        });
      }
    });
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute bottom-[105%] left-1/2 -translate-x-1/2 w-72 bg-white/95 border border-slate-200 rounded-2xl p-4 shadow-2xl backdrop-blur-xl z-50 transition-all duration-300 pointer-events-none ${
          ...
          <div className="space-y-2">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">{hoveredLocation.country}</p>
              </div>
            ...
            <div className="border-t border-slate-100 pt-2">
              ...
              <p className="text-[11px] text-slate-600 italic mt-1 font-sans leading-relaxed">
                "{hoveredLocation.fact}"
              </p>
            </div>

            <div className="flex justify-between items-center bg-slate-50 px-2.5 py-1.5 rounded-lg text-[10px] font-mono text-slate-500 mt-1">
      ...
      >
        ...
        <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-[#ff4e17]/5 via-transparent to-black/5 blur-xl pointer-events-none" />

        <svg
          width={width}
          height={height}
          viewBox={`0 0 ${width} ${height}`}
          className="relative drop-shadow-[0_4px_12px_rgba(15,23,42,0.08)]"
        >
          ...
          <circle cx={cx} cy={cy} r={radius} fill="url(#sphere-gradient)" stroke="rgba(15, 23, 42, 0.08)" strokeWidth="1" />
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  stroke={isHovered ? loc.color : "rgba(0,0,0,0.2)"}
        ...
        <div className="absolute bottom-1 right-2 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
          <span className="text-[10px] font-mono text-slate-600 bg-white/80 border border-slate-200 px-2 py-0.5 rounded-full backdrop-blur-md flex items-center gap-1">
